...
import { ALUserModelGetAll, ALUserQueryparams } from '../aluser.model';
...
export class ALUserListService {
   ...
   */
  constructor(private _httpClient: HttpClient,private Router:Router,private ALRoleListService:ALRoleListService) {
    // Set the defaults
  ...
  resolve(route: ActivatedRouteSnapshot, state: RouterStateSnapshot): Observable<any> | Promise<any> | any {
   ...
   this.sortValue = this.queryParams.sort || ''
    return new Promise<void>((resolve, reject) => {
      Promise.all([this.getRows(route?.queryParams),this.ALRoleListService.getRows()]).then(() => {
        resolve();
      }, reject);
    });
  }
  ...
  getRows(queryParams?): Promise<ALUserModelGetAll> {
    console.log('search',queryParams);
    return new Promise((resolve, reject) => {
      this._httpClient.get(`${environment.extrnalApiUrl}/user`,{params:queryParams || this.queryParams }).subscribe((response: ALUserModelGetAll) => {
